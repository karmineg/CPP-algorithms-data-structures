...
void animalmaisNovo(No<Ficha>* animS, No<Ficha>* animD) {
    if(animS == NULL && animD == NULL) cout << "ERRO: Não há animais atendidos em 'MENORIDADE'";
        ...
        cout << "Animal(is) selvagem(ns) mais novo(s) em estado grave(s)\n";
        cout << "            Nome |           Idade |       Estado \n";
        cout << "-----------------|-----------------|--------------\n";
        while(P != NULL) {
            Ficha k = P->dado;
            cout << setfill(' ') << std::setw(16) << k.nome << " |";
            cout << setfill(' ') << std::setw(16) << k.idade << " |";
            cout << setfill(' ') << std::setw(14) << k.estado << endl;
            P = P->prox;
        }

        cout << endl;
        cout << "Animal(is) doméstico(ns) mais novo(s) em estado grave(s)\n";
        cout << "            Nome |           Idade |       Estado \n";
        cout << "-----------------|-----------------|--------------\n";
        while(D != NULL) {
            Ficha u = D->dado;
            cout << setfill(' ') << std::setw(16) << u.nome << " |";
            cout << setfill(' ') << std::setw(16) << u.idade << " |";
            cout << setfill(' ') << std::setw(14) << u.estado << endl;
            D = D->prox;
        }
    }
}
                ...
                if(Fn != NULL) {
                    ...
                    if(qtd <= 0) cout << "ERRO: Remova, pelo menos, uma ficha da fila";
                        ...
                        while(Fp != NULL && QTD < qtd) {
                            ...
                            cout << "\nO animal " << F.tipo << " de nome '" << F.nome << "' em estado " << F.estado << " foi atendido" ;
                            Fp = removeF(Fp);
                            QTD++;
                        } C += QTD;
