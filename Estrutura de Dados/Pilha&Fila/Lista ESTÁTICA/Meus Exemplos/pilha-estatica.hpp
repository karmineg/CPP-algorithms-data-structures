...
template<typename T> void mostrarSimplesP(Pilha<T> *p, string nome)
{
    cout << "Pilha " << nome << ": ";
    if(vaziaP(p)) cout << "VAZIA";
    else {
        for(int i = p->base + 1; i <= p->topo; i++) {
            cout << p->dados[i] << ", ";
        }
    }
}
...
template<typename T> void ordenaP(Pilha<T> *p, bool cresc) {
    ...
    while(true) {
        ...
        for(int i = 0; i < p->topo; i++) {
            if((D[i] > D[i + 1] && cresc) || (D[i] < D[i + 1] && !cresc)) {
                T aux = D[i + 1];
                D[i + 1] = D[i];
                D[i] = aux;
                ord = false;
